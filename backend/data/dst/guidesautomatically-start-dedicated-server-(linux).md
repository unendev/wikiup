...
source_url: https://dontstarve.fandom.com/wiki/Guides/Automatically_Start_Dedicated_Server_(Linux)
---
...
If you have not yet setup a dedicated server on a Linux machine I would recommend using the following as reference. This was used to setup DST on my Linux server: [http://dont-starve-game.wikia.com/wiki/Guides/Simple\_Dedicated\_Server\_Setup](https://dontstarve.fandom.com/wiki/Don’t_Starve_Together_Dedicated_Servers)

You'll need to create the file in your /etc/init.d/ directory (command supplied below). If you followed the above guide then the directories should be the same. I named the file "start\_dst" for my own reference you may change it "name".sh with out the quotes.
